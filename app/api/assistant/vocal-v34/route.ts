 ...
 * V34: NUMERICAL INTEGRITY & LIST BAN (Fixed Hundreds)
 */
...
function vocalPurifyV34(text: string): string {
    // V34: LIST BAN & PURE NARRATIVE
    let result = text
        /**
        ...
        /**
         * 1. LIST MARKER PURGE (Fallback against prompt violations)
         * Strips "1. ", "1: ", "- " from the start of lines or after punctuation.
         */
        .replace(/(^|[.!?\n])\s*\d+[.:-]\s*/g, '$1 ')
        .replace(/(^|[.!?\n])\s*[-•]\s*/g, '$1 ')

        /**
         * 2. THE BRAZILIAN PRONUNCIATION ARMOR
         */
        .replace(/^Em\b/gi, 'Êm')
        ...
        /**
         * 3. ANALYTICAL SLOWDOWN for Numbers
         */
...
export async function POST(req: Request) {
    try {
        ...
        const cleanText = vocalPurifyV34(text);

        // V34: NO STUTTER - Pure narrative flow.
        const phoneticText = ` . . . ${cleanText} . . . `;

        console.log(`\n##################################################`);
        console.log(`[VOCAL-v34-MASTER] Path: /api/assistant/vocal-v34`);
        console.log(`[VOCAL-v34-MASTER] Original: "${text.substring(0, 50)}..."`);
        console.log(`[VOCAL-v34-MASTER] Phonetic: "${phoneticText.substring(0, 250)}..."`);
        console.log(`##################################################\n`);
        ...
        console.error('[VOCAL-v34-MASTER] error:', error);
